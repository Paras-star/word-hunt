...
// 15 categories in the exact order defined by the specification.
export const CATEGORIES: Category[] = [
  ...
  },
  {
    ...
    words: [
      ...
      'BROCCOLI', 'SPINACH', 'CABBAGE', 'CELERY', 'RADISH', 'PUMPKIN', 'ZUCCHINI',
    ],
  },
];
...
/**
 * Level i is unlocked iff i === 0 or CATEGORIES[i-1].id is in completedLevels.
 */
export function isCategoryUnlocked(index: number, completedLevels: string[]): boolean {
  if (index <= 0) return true;
  const prev = CATEGORIES[index - 1];
  return !!prev && completedLevels.includes(prev.id);
}
